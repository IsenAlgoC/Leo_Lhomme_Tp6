...
{
	if (_strcmpi(enr1.nom, enr2.nom) == 0 && _strcmpi(enr1.prenom, enr2.prenom) == 0 && _strcmpi(enr1.tel, enr2.tel) > 0) {     
		return(false);
...
#ifdef IMPL_TAB
	// ajfopen_s(&fic_rep, nom_fichier, "w");  // ouverture fichier
	...
	else
	{
		while (!feof(fic_rep) && (rep->nb_elts < MAX_ENREG))
		{
			if (fgets(buffer, long_max_rec, fic_rep) != NULL)
			{
				...
				InsertElementAt(rep->liste, num_rec, nouveau); #endif
#endif
#endif
